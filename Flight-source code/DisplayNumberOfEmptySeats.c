...
#include "DisplayNumberOfEmptySeats.h"
#include "Structure.h"
//***************************************************************************
...
void DisplayNumberOfEmptySeats(CA* Seats)
{
	int Availableseats = MIN;
	for (int i = MIN;i < MAXSEATS;i++)                                      // for loop for checking all seats
	{
		if (Seats[i].Assign == 'N')
		{
			Availableseats++;                                               //keep an count on available seats
		}

	}
	printf("The Available seats in the airplane are: %d\n", Availableseats);    //prints available seats
